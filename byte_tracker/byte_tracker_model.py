...
def tlwh_to_tlbr(bbox):
    """
    Convert bounding box from top-left width-height (tlwh) format to top-left bottom-right (tlbr) format.
    """
    x, y, w, h = bbox
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, class_name,original_tlbr=None):

        # wait activate
        self.original_tlbr = original_tlbr  # keep detector box
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        self.original_tlbr = new_track.original_tlbr
...
class BYTETracker(object):
    ...
    def update(self, output_results, xyxy=True):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [
                STrack(STrack.tlbr_to_tlwh(tlbr), s, c, original_tlbr=tlbr)
        ...
        # if xyxy:
            # output_stracks = [[tlwh_to_tlbr(track.tlwh), track.score, track.class_name] for track in output_stracks]
